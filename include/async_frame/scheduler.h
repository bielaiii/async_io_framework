...
public:
  ...
  void submit_read(int fd, fd_ops *op, use_awaiter_t) {
    ...
    submit_event_impl(fd, op, std::to_underlying(register_type::EVENT_READ));
  }

  void submit_connect(int fd, fd_ops *op, use_awaiter_t) {
    submit_event_impl(fd, op, std::to_underlying(register_type::EVENT_CONNECT));
  }

  void submit_accept(int fd, fd_ops *op, use_awaiter_t) {
    submit_event_impl(fd, op, std::to_underlying(register_type::EVENT_ACCEPT));
  }

  void submit_write(int fd, fd_ops *op, use_awaiter_t) {

    submit_event_impl(fd, op, std::to_underlying(register_type::EVENT_WRITE));
  }

  void submit_timer(int fd, fd_ops *op, use_awaiter_t) {

    submit_event_impl(fd, op, std::to_underlying(register_type::EVENT_TIMER));
  }
